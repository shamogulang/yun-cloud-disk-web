...
const handleFileChange = async (file: any) => {
  if (!file) return
  // 添加到传输列表，并获取任务ID
  const taskId = transferStore.addTask({ name: file.name, size: file.size })
  // 弹出提示
  ElMessage.success('已添加至上传列表')
  try {
    loading.value = true
    ...
    await uploader.upload((progress) => {
      ...
      const deltaBytes = (deltaProgress / 100) * file.size
      const deltaTime = (now - lastTime) / 1000
      const speed = deltaTime > 0 ? deltaBytes / deltaTime : 0
      lastProgress = progress
      lastTime = now
      // 响应式更新 transferStore，使用任务ID查找
      const task = transferStore.tasks.find(t => t.id === taskId)
      if (task) {
        ...
        transferStore.tasks.splice(transferStore.tasks.indexOf(task), 1, { ...task })
      }
    })
    // 上传成功后刷新文件列表
    await loadFileList()
